...
import {blobToUint8Array, formatTime, strLengthLimit} from "@/utils";
import {Message, MessageType, messageTypeToJSON, Status} from "@/proto/message";
...
import {artplayerStreamDanmu, sendDanmu} from "@/plugins/danmu";
...
const playerOption = computed<options>(() => {
    ...
    let option: options = {
        ...
        plugins: [
            /**
             * TODO
             * 弹幕重复发送修改完成 必须这么获取dom 放外层获取不到dom
             * 房间设置 增加修改发送上限
             **/
            artplayerPluginDanmuku({
                ...
                async beforeEmit(danmu: any) {
                    const danmukuSender: HTMLInputElement = document.querySelector('.apd-input')
                    console.log(danmukuSender.value)
                    const num = 4096
                    // 按 Enter  发个友善的
                    console.log(danmu)
            ...
            <div class="card h-full">
                <div class="card-title">
                    ...
                    <el-button
                            ...
                            size="small"
                            style="float: right"
                    >退出语音 ({{ peerConnectionsLengthWithUserId }})
                    </el-button
                    >
                </div>
                <div
